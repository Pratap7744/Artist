...
// Use Vite's import.meta.glob to get all assets
const makeupImages = import.meta.glob('../src/assets/{ff,aa,a,b,dd,c,d,gg,bb,e,ee,f,g,ff,h,cc,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z}.jpg', { eager: false });
const embroideryImages = import.meta.glob('../src/assets/{a1,a2,a3,a4,a5,a6,a7,a8,a9}.jpg', { eager: false });
const hairstyleImages = import.meta.glob('../src/assets/{h19,h1,h2,h3,h4,h5,h6,h7,h8,h9,h10,h11,h12,h13,h14,h15,h16,h17,h18}.jpg', { eager: false });
const videoFiles = import.meta.glob('../src/assets/{vid1,vid2,vid3}.mp4', { eager: false });

// Function to create arrays of image paths in the correct format
const generateAssetPaths = () => {
  return {
    makeup: Object.keys(makeupImages),
    embroidery: Object.keys(embroideryImages),
    hairstyles: Object.keys(hairstyleImages),
    videos: Object.keys(videoFiles)
  };
};

// Improved image loader component that handles all image types
const LazyImage = ({ src, alt, className }) => {
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      try {
        // Check which module collection contains this path
        let moduleGetter;
        if (src in makeupImages) {
          moduleGetter = makeupImages[src];
        } else if (src in embroideryImages) {
          moduleGetter = embroideryImages[src];
        } else if (src in hairstyleImages) {
          moduleGetter = hairstyleImages[src];
        } else {
          console.error("Image not found in any collection:", src);
          return;
        }
...
// Improved video loader component
const LazyVideo = ({ src, className, controls, onClick, videoRef }) => {
  ...
  useEffect(() => {
    ...
    const loadVideo = async () => {
      try {
        if (src in videoFiles) {
          const module = await videoFiles[src]();
          if (isMounted) {
...
const BeautyAndEmbroideryWebsite = () => {
  ...
  // Generate image paths once at component initialization
  const imagePaths = useRef(generateAssetPaths());
  ...
  useEffect(() => {
    ...
    // Load videos
    setTimeout(() => {
      setVideos(imagePaths.current.videos.map((src, index) => ({
        id: index + 1,
        src: src
      })));
      setLoading(false);
    ...
    // Load hairstyles
    setTimeout(() => {
      setHairstyles(imagePaths.current.hairstyles);
    }, 500);
  ...
  // Reference to portfolio images
  const portfolioImages = {
    makeup: imagePaths.current.makeup,
    embroidery: imagePaths.current.embroidery,
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      ...
      {/* Hair Styles Section - Fixed for proper image loading */}
